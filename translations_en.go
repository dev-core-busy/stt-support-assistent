...
var enOverrides = map[string]string{
	...
	"Hinweis: Die Jarvis-API muss noch angepasst werden, damit mit diesen Schlagworten auch dort passende Tickets gesucht werden können. Die Kundenverwaltung (tickets-by-buzzwords) wird bereits abgefragt, sofern ein Anruf eine Kundenv.-ID geliefert hat.": "Note: The Jarvis API still needs to be adapted so that matching tickets can also be searched there using these keywords. Customer management (tickets-by-buzzwords) is already queried, provided a call supplied a customer-mgmt ID.",
